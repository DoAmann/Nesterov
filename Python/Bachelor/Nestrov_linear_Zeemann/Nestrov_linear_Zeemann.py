...
q = 0.1 # quadratic Zeeman shift units in Hz
p = 2.35 # linear Zeeman shift units in Hz
mu_zeeman = 2.7e-32 # chemical potential
...
Restart = 10000                     #for the condition of restarting
...
while np.max([e_0, e_neg, e_pos])>tol and i < ITER:
    ...
    e_0 = np.sqrt( (ddx / N_points) * np.sum(np.abs((1/2) * Lap * FT_Psi2_0 + FT_PsiX_0)**2))
    e_neg = np.sqrt( (ddx / N_points) * np.sum(np.abs((1/2) * Lap * FT_Psi2_neg + FT_PsiX_neg)**2))
    e_pos = np.sqrt( (ddx / N_points) * np.sum(np.abs((1/2) * Lap * FT_Psi2_pos + FT_PsiX_pos)**2))
